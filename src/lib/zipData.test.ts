...
// truncates back to a stub or drops one of the 8 target states.
//
...
describe('zip-greatlakes.json coverage', () => {
  ...
  // Every one of the 8 covered states must have *some* ZIPs — catches a
  // half-broken pipeline that quietly drops a state.
  it('covers all 8 target states with at least 100 ZIPs each', () => {
    ...
    for (const state of ['MN', 'WI', 'IL', 'IN', 'MI', 'OH', 'PA', 'NY']) {
